...
func (_q *PostQuery) QueryGoal() *GoalQuery {
...
func (_q *PostQuery) QueryReactions() *ReactionQuery {
...
func (_q *PostQuery) WithGoal(opts ...func(*GoalQuery)) *PostQuery {
	query := (&GoalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) WithReactions(opts ...func(*ReactionQuery)) *PostQuery {
	query := (&ReactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if _q.withUser != nil || _q.withGoal != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withGoal; query != nil {
		if err := _q.loadGoal(ctx, query, nodes, nil,
			func(n *Post, e *Goal) { n.Edges.Goal = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *PostQuery) loadGoal(ctx context.Context, query *GoalQuery, nodes []*Post, init func(*Post), assign func(*Post, *Goal)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Post)
	for i := range nodes {
		if nodes[i].goal_posts == nil {
			continue
		}
		fk := *nodes[i].goal_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "goal_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PostQuery) loadReactions(ctx context.Context, query *ReactionQuery, nodes []*Post, init func(*Post), assign func(*Post, *Reaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
