...
func (_q *ImageQuery) QueryUploadedBy() *UserQuery {
...
func (_q *ImageQuery) WithUploadedBy(opts ...func(*UserQuery)) *ImageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if _q.withPost != nil || _q.withUploadedBy != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *ImageQuery) loadUploadedBy(ctx context.Context, query *UserQuery, nodes []*Image, init func(*Image), assign func(*Image, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Image)
	for i := range nodes {
		if nodes[i].user_uploaded_images == nil {
			continue
		}
		fk := *nodes[i].user_uploaded_images
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_uploaded_images" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
